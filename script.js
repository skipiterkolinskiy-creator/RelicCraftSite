...
const cleanPath = isFilePage
  ? window.location.pathname.replace(/\/(start|features|voice|rules|discord|bank)\/\1\.html$/i, "/$1/index.html")
  : window.location.pathname
      .replace(/\/index\.html$/i, "/")
      .replace(/\/(start|features|voice|rules|discord|bank)\/\1\.html$/i, "/$1/");
  ...
  link.addEventListener("click", (event) => {
    ...
    if (isFilePage) {
      if (href === "./") {
        ...
        href = href.replace(/(^|\/)(start|features|voice|rules|discord|bank)\/([?#].*)?$/i, "$1$2/index.html$3");
      }
    }
